    ...
    <div className="w-10/12 lg:w-8/12 mx-auto form-container">
      ...
      <div className="relative flex flex-col">
        ...
        {data.map((form) => (
          <div
            key={form.id}
            className="relative flex flex-col-reverse md:flex-row items-center mb-10 w-full fade-form md:ml-2.5"
          >
            <div className="w-full md:w-3/4 md:pr-10 text-right">
              {activeId === form.id ? (
                <div className="bg-white shadow-md rounded-lg p-6 border-r-4 border-green-500 text-left md:text-right">
                  ...
                  <p className="text-sm">
                    <strong>Spécialité:</strong> {renderList(form.specialite)}
                  </p>
                </div>
              ) : (
                <button
                  onClick={() => toggleInfo(form.id)}
            ...
            <div className="w-full md:w-1/4 text-sm font-text text-center md:text-left md:pl-4 mb-2 md:mb-0">
              {form.date}
            </div>
          </div>
        ))}
